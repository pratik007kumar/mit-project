...
if(!$crop -> getMsg())
{
    ...
    // print_r($imgarr);
   // echo $imgarr['2'];
    $org=$imgarr['0']."/upload/".$imgarr['2'];


   //echo $type =".jpg" exif_imagetype($path);
    $imgprefix= date('YmdHis').".jpg";

    rename($org, "../../uploades/profile/org/".$imgprefix);
    img_resize( $path , 40 , "../../uploades/profile/40/" , $imgprefix);
    img_resize( $path , 120 , "../../uploades/profile/120/" , $imgprefix);
    rename($path, "../../uploades/profile/220/".$imgprefix);

    updateProfileImage($_SESSION['id'],$imgprefix);
//echo $crop -> getResult();
//exit;
}
...
$response = array(
    'state'  => 200,
    'message' => $crop -> getMsg(),
    'result' => SITE_URL."uploades/profile/120/".$imgprefix
);
